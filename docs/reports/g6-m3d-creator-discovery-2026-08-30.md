...
refresh happens roughly **once every four hours per active user** instead of
once per JOIN. The marginal exposure is one short-lived credential that is
...
| Access-token lifetime | Bounded by `expires_in` (≈4h in practice) |
| Refresh-token expiry | Public clients expire in 30 days; **confidential clients do not expire**. Watchside is confidential, so a stored credential stays valid indefinitely — *more* dangerous, not less |
...
because confidential-client refresh tokens never expire, deletion is the *only*
thing that ends custody; time will not do it.
...
| 9 | Confidential refresh tokens never expire | Medium | Deletion is the only end; all three triggers delete (§74) | **Inherent to Twitch.** Time will not clean up after us |
