...
**Status:** **ACTIVATED 2026-09-09 - migration applied, website deployed, production E2E PASSED.** No ad spend.

*Runtime state: the Twitch application migration is closed and production is healthy. See PRODUCTION ACTIVATION at the end of this report.*
...
# PRODUCTION ACTIVATION — Acquisition V1 is operational

**2026-09-09.** Applied, deployed, and proven end to end against production.
**No ad spend. Reddit Ads inactive.**

## Migration 0045 — applied

```
db push --dry-run   -> only 0045
db push             -> Applying migration 0045_acquisition_provider_metadata.sql
migration list      -> local and remote aligned through 0045, 0 unapplied
re-push --dry-run   -> "Remote database is up to date"
analytics_schema_version() = 45
```

Columns `provider` / `medium` / `content` / `term` present, all `text`, all
nullable. All eight reporting views present, including `acquisition_activation_v`.

**Authorization, verified against production twice over.** `information_schema`
reports **0** grants on any `%_v` to `anon`, `authenticated` or `PUBLIC`; and a
live PostgREST probe using the publishable key that ships inside the extension
returns **HTTP 401** for all seven acquisition/activation relations. That is the
0043 stop-ship class tested against production itself, not against a harness.

**Immutability, live.** A `provider` update on a real campaign row was rejected
by the trigger and rolled back; the value is still `reddit`.

## Campaign registry

Three rows, generated by `scripts/campaign.mjs` from the committed manifest
rather than hand-typed, so the definitions could not drift.

| code | source | provider | medium | content | active |
| --- | --- | --- | --- | --- | --- |
| `reddit-launch-a` | reddit | reddit | paid_social | `explanatory_presence` | true |
| `reddit-launch-b` | reddit | reddit | paid_social | `social_gravity` | true |
| `reddit-launch-c` | reddit | reddit | paid_social | `join_payoff` | true |

## Website — deployed

`Anoteros-Labs/watchside-app` `5724c39..768d762`. Verified against the **public
URLs**, not the build output.

`/`, `/privacy`, `/support`, all three `/c/<code>/`, `/js/route.js` and
`/js/campaign.js` all return **200**. `/i/<code>` and an unknown `/c/` code
return 404 with the fallback page, unchanged by design.

Every store link on every campaign page carries the correct `utm_source=reddit`,
`utm_medium=paid_social`, `utm_campaign=<code>`, `utm_content=<content>`. Four
`target="_blank" rel="noopener"` per campaign page and **zero `noreferrer`**. CSP
live as designed, with `script-src 'none'` on `/privacy`. No pixel, no analytics,
no cookie, no storage, no beacon, no network call from any script. The root page
carries the organic `site_landing` tag with no campaign leakage, and `route.js`
can still build only two destinations, both `twitch.tv` literals.

## The end-to-end attribution test

Two halves, run separately, for a reason worth recording.

### Pre-auth leg — the released v0.9 artifact

Driven against the live site with the **submitted Store artifact**, extracted
read-only (hash `a6dd55b5ae6de466...` identical before and after):

```
/c/reddit-launch-a/            HTTP 200, full landing content
  store CTA clicked         -> new tab; CAMPAIGN PAGE SURVIVED
  continue promoted         -> "Installed? Continue to Twitch to finish setting up."
  Continue to Twitch        -> twitch.tv/?watchside_campaign=reddit-launch-a
  released v0.9 content script read it
  worker persisted          -> watchside:campaignTouch = reddit-launch-a (8.3s old)
```

That is the step the original investigation flagged as most likely to silently
eat a paid campaign's attribution, demonstrated working.

### A finding worth recording: the Store artifact cannot be side-loaded

The sign-in leg could **not** be completed in that browser, and the reason is a
property of the artifact rather than a defect:

```
extension ID when side-loaded : hinoafkjiknlkihiddaodohneldjechk
the real Watchside ID         : ngfopkeokddfnncdhfkhnffilbdhkkip
```

**The Store zip carries no `key` field** — verified: `key present: false`, where
`dist/manifest.json` has `key present: true`. The Chrome Web Store assigns the
identity, so the packaged artifact does not carry one, and Chromium derives an ID
from the load path instead. `chrome.identity.getRedirectURL()` therefore produced
an address that is not in Supabase's redirect allow-list and never will be.
**OAuth cannot complete in a side-loaded Store build.** Worth knowing before
anyone plans another end-to-end test around one.

### Authenticated bind leg — the real AMO install

Performed by the owner in their ordinary Firefox, already signed in as
AnoterosTV, real AMO build, no reinstall and no new profile. Visiting the
campaign page and clicking Continue is sufficient: `noteCampaignTouch` binds
immediately when the worker is already authenticated.

```
acquisition_attribution
  actor                e9ee4788... (AnoterosTV)
  first_campaign_code  reddit-launch-a
  last_campaign_code   reddit-launch-a
  touch_count          1
  first_touch_at = last_touch_at   (a single touch)

analytics_events
  acquisition_attributed   { "touch": "first", "source": "reddit" }
  environment private_beta, occurred_at identical to the attribution row
  acquisition_touch_rejected: 0
```

`source: "reddit"` was **not** supplied by the client — `bind_acquisition` copies
it from the registry server-side. The client sent a code and nothing else, which
is the whole defence.

**First-touch immutability, live:** an attempt to change `first_campaign_code`
was rejected by the trigger and rolled back; the row is unchanged.

### Internal-actor exclusion — a pass, not a gap

AnoterosTV is `is_internal = true`, so every reporting view correctly shows
nothing:

```
acquisition_actor_v 0 · acquisition_campaign_v 0 · acquisition_activation_v 0
acquisition_downstream_v 0 · acquisition_touch_outcomes_v 0
```

Empty could mean "exclusion works" or "the views are broken", so it was
disambiguated rather than assumed. The identical join **without** the internal
filter returns the row, fully enriched from the registry:

```
actor e9ee4788... | reddit-launch-a | is_internal true
provider reddit   | medium paid_social | content explanatory_presence
```

And `acquisition_coverage_v` holds **4** rows from pre-existing non-internal
actors, so the view machinery is demonstrably live. **`is_internal` is the sole
cause of the exclusion.**

A non-internal user is therefore not required to establish correctness. The first
genuinely acquired user will populate the views on their own.

### Nothing duplicated, nothing unexpected

```
attribution_rows 1 · attributed_events 1 · rejected_events 0
campaigns 3 · active_campaigns 3
users_total 9 · users created by the test 0
```

## Verdict

**Acquisition V1 is end-to-end operational in production.** Campaign URL →
landing page → store handoff → Twitch → released extension → authenticated bind
→ immutable first-touch attribution → server-resolved provider metadata, with
internal-actor exclusion and view authorization both proven live.

All four go/no-go conditions from §19 are now met: Twitch integration healthy,
0045 applied, site deployed, and one real production bind observed. Reddit Ads
remain inactive pending creative assets.
